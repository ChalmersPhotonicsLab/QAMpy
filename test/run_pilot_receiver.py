...
for l in range(npols):
    ...
    corr_pilots[l,:] = pilotbased_receiver.correct_const_phase_offset(comp_test,phase_offset)

# Equalize the signal
comp_test_sig = []
for l in range(npols):
    test_sig = equalisation.apply_filter(SS[:,shift_factor[l]:shift_factor[l]+frame_length*os+Numtaps-1],os,taps[l])   
    comp_test = pilotbased_receiver.correct_const_phase_offset(phaserecovery.comp_freq_offset(test_sig[l,:],foePerMode[l]), phase_offset[l])
...
phase_trace = []
for l in range(npols):   
    symbs, trace = pilotbased_receiver.pilot_based_cpe(comp_test_sig[l][:,pilot_seq_len:], pilot_symbs[l,pilot_seq_len:], pilot_ins_ratio,use_pilot_ratio=1, num_average = 5, max_num_blocks = None)
