...
Ex, Ey, wx, wy, err, err_rde = equalisation.FS_CMA_RDE_16QAM(SS[0], SS[1], 10000, 40000, 30, 2, 0.0001, 0.0003)


evmX = cal_blind_evm(X[::2], M)
evmY = cal_blind_evm(Y[::2], M)
evmEx = cal_blind_evm(Ex[::2], M)
evmEy = cal_blind_evm(Ey[::2], M)
#sys.exit()
plt.figure()
plt.subplot(131)
plt.title('Recovered')
plt.plot(Ex.real, Ex.imag, 'r.' ,label=r"$EVM_x=%.1f\%%$"%(evmEx*100))
plt.legend()
plt.subplot(132)
plt.plot(Ey.real, Ey.imag, 'g.', label=r"$EVM_y=%.1f\%%$"%(100*evmEy))
plt.legend()
plt.subplot(133)
plt.title('Original')
